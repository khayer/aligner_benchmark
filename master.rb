...
def run_hisat(options, source_of_tree, dataset)
  ...
  Dir.glob("#{l}/*").each do |p|
    if File.directory? p
      next unless File.exist?("#{p}/output.sam")
      $logger.debug(p)
      options[:stats_path] = "#{options[:out_directory]}/hisat/#{p.split("/")[-1]}".gsub(/[()]/,"")
      begin
        Dir.mkdir(options[:stats_path])
      rescue SystemCallError
        if Dir.exist?(options[:stats_path])
      ...
      options[:tool_result_path] = p
      shell_file = "#{options[:jobs_path]}/hisat_statistics_#{options[:species]}_#{dataset}_#{p.split("/")[-1]}.sh".gsub(/[()]/,"")
    else
      next unless p =~ /output\.sam$/
      $logger.debug(p)
      options[:stats_path] = "#{options[:out_directory]}/hisat/".gsub(/[()]/,"")
      begin
        Dir.mkdir(options[:stats_path])
      rescue SystemCallError
        if Dir.exist?(options[:stats_path])
      ...
      options[:tool_result_path] = p.gsub(/output\.sam$/,"")
      shell_file = "#{options[:jobs_path]}/hisat_statistics_#{options[:species]}_#{dataset}_default.sh"
    end
