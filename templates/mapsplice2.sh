#!/bin/bash -e
#BSUB -J mapsplice2_stats<%= @run_name %>              # LSF job name
#BSUB -o mapsplice2_stats<%= @run_name %>.%J.out       # Name of the job output file
#BSUB -e mapsplice2_stats<%= @run_name %>.%J.error     # Name of the job error file

cd <%= @stats_path %>

grep -v "^@" <%= @tool_result_path %>/alignments.sam | sort -t'.' -k 2n > output.sam
