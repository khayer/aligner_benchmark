...
#
# IN:
# [hayer@consign aligner_benchmark]$ ls */comp_res_multi_mappers.txt
#     gsnap/comp_res_multi_mappers.txt  hisat/comp_res_multi_mappers.txt  mapsplice2/comp_res_multi_mappers.txt
# OUT: Summary
#
###
all = []
...
ARGV[0..-1].each do |arg|
  info = []
  info << arg.gsub(/([\.\/]|comp_res_multi_mappers.txt$)/,"")
...
names.each_with_index do |name, j|
  ...
  when borders[2]
    puts "---------------- JUNC LEVEL ---------------------"
  when borders[3]-2
    puts "---------------- READ LEVEL PAIRS ---------------------"
  when borders[4]-2
    puts "---------------- BASE LEVEL PAIRS ---------------------"
  end
end
